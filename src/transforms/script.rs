...
#![allow(clippy::manual_strip, clippy::collapsible_if, clippy::collapsible_else_if)]
...
impl ScriptTransform {
    ...
    fn parse_assignment(line: &str) -> Result<Option<ScriptOp>> {
        ...
        if right.starts_with("split(") && right.ends_with(')') {
            ...
            if args.len() == 2 {
                ...
                if src_arg.starts_with('.')
                    && delim_arg.starts_with('"')
                    && delim_arg.ends_with('"')
                {
    ...
    fn parse_statement(line: &str) -> Result<Option<ScriptOp>> {
        ...
        if line.starts_with("del(") && line.ends_with(')') {
            ...
            if inner.starts_with('.') && inner.len() > 1 {
                return Ok(Some(ScriptOp::DelField {
                    field: inner[1..].to_string(),
                }));
            }
        }
                ...
                ScriptOp::UpcaseField { field, src } => {
                    if let Some(log) = event.as_log_mut() {
                        ...
                        if let Some(Value::String(s)) = val {
                            log.fields
                                .insert(field.clone(), Value::String(s.to_uppercase()));
                        }
                    }
                }
...
mod tests {
    ...
    fn parse_assignment_extended_funcs() {
        // now()
        let op = ScriptTransform::parse_assignment(".ts = now()")
            .unwrap()
            .unwrap();
        match op {
        ...
        // to_float
        let op = ScriptTransform::parse_assignment(".f = to_float(.x)")
            .unwrap()
            .unwrap();
        match op {
            ScriptOp::ToFloat { field, src } => {
        ...
        // to_string
        let op = ScriptTransform::parse_assignment(".s = to_string(.x)")
            .unwrap()
            .unwrap();
        match op {
            ScriptOp::ToString { field, src } => {
        ...
        // parse_timestamp
        let op = ScriptTransform::parse_assignment(".ts = parse_timestamp(.raw)")
            .unwrap()
            .unwrap();
        match op {
            ScriptOp::ParseTimestamp { field, src } => {
